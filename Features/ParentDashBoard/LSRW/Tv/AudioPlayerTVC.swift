...
class AudioMessageView: UIView {
    ...
    var playDurationUpdate: ((Double) -> Void)?
    var isPlaying = false {
        didSet {
            updatePlayingState()
        }
    }
    private var pendingPlaybackPosition: Double?
    ...
    func setupAudioUrl(_ url: URL?,_ duration:Double? = 0.0) {
        reset()
        ...
        pendingPlaybackPosition = duration
        audioURL = url
        loadAudio()
    }
            ...
            DispatchQueue.main.async {
                if status == .loaded {
                    ...
                    if duration.isFinite && duration > 0 {
                        ...
                        if let pendingPosition = self.pendingPlaybackPosition {
                            self.applyPlaybackPosition(pendingPosition, totalDuration: duration)
                        }
                    } else {
                        self.durationLabel.text = "0:00"
                    }
    ...
    func setPlaybackPosition(_ duration: Double) {
        ...
        applyPlaybackPosition(duration, totalDuration: totalDuration)
    }
    
    private func applyPlaybackPosition(_ duration: Double, totalDuration: Double) {
        guard let player = audioPlayer else { return }
        
        let validDuration = max(0, min(duration, totalDuration))
        let seekTime = CMTime(seconds: validDuration, preferredTimescale: 600)
        
        player.seek(to: seekTime, toleranceBefore: .zero, toleranceAfter: .zero) { [weak self] finished in
            guard let self = self, finished else { return }
            let progress = validDuration / totalDuration
            let remainingTime = totalDuration - validDuration
            self.durationLabel.text = self.formatTime(remainingTime)
    ...
    private func updateUI() {
        ...
        playDurationUpdate?(currentTime)
        updateWaveformColor(progress: progress)
    }
