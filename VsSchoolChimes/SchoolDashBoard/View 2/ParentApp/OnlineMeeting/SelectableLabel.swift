...
final class SelectableLabel: UILabel {
    ...
    private func menuForSelection() -> UIMenuController {
        ...
        menu.menuItems = [
           // UIMenuItem(title: "Copy", action: #selector(copyText)),
            UIMenuItem(title: "Paste", action: #selector(pasterText)),
        ]
        return menu
    }
    ...
    @objc private func pasterText(_ sender: Any?) {
        cancelSelection()
        let board = UIPasteboard.general.string ?? ""
        print("Paste : \(String(describing: board))")
        let nc = NotificationCenter.default
       
            nc.post(name: Notification.Name(rawValue:"pasteLabelNotification"), object: board)
