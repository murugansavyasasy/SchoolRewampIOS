...
class ExamTestSchoolSelectVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
...
//                        cell.locationTop.constant = 4
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "ExamTestSegue", sender: self)
        
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if (segue.identifier == "ExamTestSegue")
        {
            let segueid = segue.destination as! ExamTestVC
